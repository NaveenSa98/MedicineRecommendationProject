...
class MedicineRecommendationSystem:
    ...
    def predict_disease(self, patient_symptoms):
        ...
        invalid_symptoms = [sym for sym in patient_symptoms if sym not in self.symptoms_dict]
        if invalid_symptoms:
            raise ValueError(f"Invalid symptoms: {invalid_symptoms}")
        ...
        # Predict disease
        predicted_disease = self.diseases_list[self.svc.predict([input_vector])[0]]
    ...
    def get_precautions(self, disease):
        ...
        return [col for col in prec.values[0]] if len(prec) > 0 else []

    def get_medicines(self, disease):
        """Get recommended medicines"""
        med = self.df_med[self.df_med['Disease'] == disease][['Medication']]
        return [m[0] for m in med.values] if len(med) > 0 else []

    def get_diets(self, disease):
        """Get recommended diets"""
        dit = self.df_dit[self.df_dit['Disease'] == disease]['Diet']
        return [d for d in dit.values] if len(dit) > 0 else []

    def get_workouts(self, disease):
        """Get recommended workouts"""
        wrkout = self.df_work[self.df_work['disease'] == disease]['workout']
        return [w for w in wrkout.values] if len(wrkout) > 0 else []